...
**Спецификация симуляции:** `mt_ca/app/` — **scenario** — условия (среда, seed); **embedding** — размерность сетки (`3+1` / `2+1`). **lab** — приборы. CLI: `list` | `run` | `panel` + `--embedding`.
...
### Дано (спецификация sim = MODEL §3.12)
...
**Численно (спецификация КА):** тот же функциональный тест без FFT-эвристики:
