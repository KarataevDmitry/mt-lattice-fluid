...
## §3. Функция перехода `g`
**Канон `g`:** §3.12. **§3.1–3.2** — каркас и ограничения до замыкания.
...
**A3 vs A6:** **`z+γΔ₄z`** не унитарна → **`local_ca`** bond-sweeps. DFT-спектр — **T-калибровка** **`ω(k)`**, не micro_step.
**Замыкание `g`:** §3.12.
### 3.4 Holonomy carrier — phase wrap

**`Arg(Σz/z)`** / **`atan2−atan2`** → ложный скачок **2π**.  
**Fix:** **`ζ = (Σ_N z)·z*`**, **`Δφ = wrap(Arg ζ)`** — carrier для §3.12.5 (не отдельный gate).
...
```
**Эволюция M:** отклонение **`Arg(⟨z⟩/z)`**; **`g`** — оператор удержания голоморфности. **Канон tick:** §3.12 (**`R(Φ)`**, saturating **`Φ`**).
| ДА | слой M |
|----|--------|
...
**Итог §3.9:** ДА = **строгий язык M**. **`g`** = **SU(2) spinor gate** (§3.10); U(1) scalar — бозонный предел.
