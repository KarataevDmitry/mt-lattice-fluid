...
**На каркасе не фиксируются:**
| параметр | статус |
|----------|--------|
| **`γ`** | не определён |
| форма **`Φ`** | не определена |
| **`L`**: unitary vs diffusive | не определён |
| **`α*`** | §7.1 |
| **`ε`** | A7; численное значение — §7.1 |
...
**A3 vs A6:** линейное **`z+γΔ₄z`** не унитарно; допустим streaming — product локальных unitaries (§3.12).
### 3.4 Holonomy carrier — phase wrap

**`Arg(Σz/z)`** / **`atan2−atan2`** → ложный скачок **2π**.  
**Fix:** **`ζ = (Σ_N z)·z*`**, **`Δφ = wrap(Arg ζ)`** — вход §3.12.5.
### 3.8 Слабые волны и вихрь
**(3+1):** FCC **N₁₂** (§1.6). **(2+1):** гекс **N₆** (§1.4).
...
```
**g** удерживает голоморфность: отклонение **`Arg(⟨z⟩/z)`**; шаг **`R(Φ)`** с saturating **`Φ`** (§3.12.5).
| ДА | M |
|----|--------|
...
#### 3.9.2 Дискретно-аналитическое уравнение (§3.12.5)
**§3.12.5:** holonomy **`ζ`** → saturating **`Φ ∈ ℤ_{N_ring}`** → **`R(Φ)`** → leapfrog **`Z⁺+Z⁻=2Z+⌊𝒩⌋`**.
| член | ДА | M (§3.12) |
|------|-----|-------|-------------|
...
| метрика **`K_P`** | saturating **`Φ`** (§3.12.5) | T readout (§7.1) |
| риманова поверхность на графе | **`Λ×S¹`** (фаза) | — |
#### 3.9.6 A9 — порог дисперсионной CR-энергии
...
| | **U(1) scalar `z∈ℂ` (бозонный предел)** | **T (e, q, …)** |
|--|-------------------------------------|-------------------------|
...
R(Φ) : (U,V) ↦ ( U·c_Φ − V·s_Φ , U·s_Φ + V·c_Φ )   (mod N)
  c_Φ = cos(2π·Φ/N_ring) , s_Φ = sin(2π·Φ/N_ring)
δZ(Φ) = R(Φ) − Z     — kick ⌊𝒩⌋ на каждой lane спинора
```
**Соответствие дискретной и непрерывной записи:**
| режим | discrete M | continuum alias |
|-------|------------|-----------------|
| **шаг g** | один **`Φ`** → **`R(Φ)`** на **`z₁`** и **`z₂`** lanes | **`exp(i·Φ·σ_z/2)`** как общая фаза (подгруппа **`U(1)_vac`**) |
| **SU(2) mix** | **`U(n,φ)`** — half-angle + Pauli | **`exp(i·φ·n·σ/2)`** — holomorphy sync, chiral; **не** один leapfrog-tick |
...
#### 3.11.3 P, C, T
Обратимость шага — **A13 / §3.12** (`g⁻¹` из leapfrog и kick ledger). Anti-unitary **`Θ = P∘C∘T`** — предел **T** (§4):
```
...
### 3.12 Закон g: обратимый CA второго порядка на ℤ_N[i]
Следствие **0.7** / **Thm 0.1**: **g** замыкает ограничения §3.2; **`p,L,F`** — **Следствие 0.8** / **Thm 5.1**.
#### 3.12.0 Из первых принципов (Planck natural units)
В **истинном M-базисе** **`hL=l_P=1`**, **`hT=t_P/√2=1`**, **`m_P=1`**, **`ρ_P=1`** (§1, §7):
| непрерывная запись | M |
|----------------------|---|
| скаляр **`z∈ℂ`** с вещественной амплитудой | **конечный бюджет** **`hV`** (Bekenstein) |
| **`z_{next}=f(z)`** + округление каждый такт | **целочисленный спинор**; фаза — дискретные сдвиги **`Δφ≥½`** |
...
M-эволюция — **обратимый CA второго порядка** на **`ℤ_N[i]`**.
...
\]
Все операции **mod `N_ring`** (§3.12.6).
...
```
**A13:** **`g⁻¹`** — leapfrog-алгебра; CPT-партнёр — §3.11.3, предел T.
...
R(\Phi) = \omega^{\Phi}\ \text{на каждой компоненте }(U,V)
\]
...
\]
+ Heisenberg floor \(|\Phi|\ge \Delta\Phi_{\min}^{\mathrm{disc}}\). **`⌊𝒩⌋`** = **`δZ(Φ) = R(Φ)−Z`** on both spinor components (same **`Φ`** → **`U(1)_vac`** on tick; полный **`U(n,φ)`** — §3.10.3).
...
**Элементарный кирпич** — **`hV = l_P³`**. Информационная ёмкость:
\[
...
Из §3.12.6 (**`N_ring=512`**) следуют законы congruence на кольце и их склейка со **звездой** **`N_{12}`**.
##### A · Лестница (4 этажа)
```
...
##### B · Законы на **`ℤ_{N_ring}`**
| id | congruence / identity | M-смысл |
|----|------------------------|---------|
| **CL-1** | **`Z⁺+Z⁻ = 2Z+⌊𝒩⌋ (mod N)`** | exact **T⁻¹** (A13) |
...
**Числа:** **`N=512`**, **`Δφ_disc=41`**, **`N_φ=13`**, **`pauli_kick=N/2=256`**, **`a_Q=2^{−6}`**, **`sync_disc=⌊41·κ_link⌋=3`** (FCC).
...
\]
**`γ=κ_link=1/|N_{12}|`** — доля звезды до проекции в **`ℤ_N`**. Требуется stencil **`N_{12}`** (§1.6).
##### D · Открытые вопросы
| leaf | вопрос | статус |
|------|--------|--------|
...
| **CL-O4** | факторизация **`N = 2^9`** vs **`N_φ=13`**, **`2^{frac_bits}`** | частично |
