...
| слой | объект | статус |
|------|--------|--------|
| **смысл** | phase↔vacuum coupling | выведено |
| **фазовый остаток** | `r = Δφ_min/(2π) = 1/(4π)` (планковская дырка) | выведено |
| **число** | фундаменталы мягкой грани (§8.2·U0) | закрыто |
...
| 2 | цикл тика `2π` | `r = Δφ_min/(2π) = 1/(4π)`; `1+r = phase_saturation` |
...
|----------|----------------------|--------|
| $M=97$ → $\kappa^{2}/(2\cdot97^{2})$ | ~−2080 | лучший int (тот же §8.2·F) |
| $M=96=N_{12}N_{\mathrm{hier}}$ | ~+1.9e4 | хуже |
| $1/(2\cdot137^{2})$ | ~+526 | **отвергнуто** — inject α_geom |
| $1/(4\pi\cdot11)^{2}/2$ | ~−1.7e4 | **отвергнуто** — FP-exp не fraction |
| $m_e N_\varphi/(2 m_H)$ | ~0 | **отвергнуто** — circular ($m_e$ уже с α) |
...
$$
**Тождество на носителе:** фазовый остаток вакуума $r=\Delta\varphi_{\min}/(2\pi)=1/(4\pi)$ (§8.2·descent) ⇒
$$
...
3. **A10:** заряд $n=\pm1$ на электронном планкон.
...
2. **Факторизация** (дверь Швингера = чтение с M):
$$
...
**Тождества на носителе:**
$$
R_\infty = \frac{\alpha^{2} m_e c}{4\pi\hbar} = \alpha^{2}\,\frac{r}{\bar\lambda_C},\qquad r=\frac{\Delta\varphi_{\min}}{2\pi}=\frac{1}{4\pi}.
$$
Тот же фазовый остаток $r$, что у Швингера; мощность $\alpha^{2}$ уже на hop-лестнице ($\alpha^{2}=N_{re}/N_{a0}$).
...
```
~260 ppm на **`α⁻¹`** vs CODATA (хуже выведено **`4π³+π²+π`** ~2 ppm). Открытый вывод: **`α_geom`** из **`g`** / Coulomb на FCC, не подменять молча.
#### §8.2·H  · **`N_a0`** (размер H в hops)
**Подход:** как §8.2·geo / §5.2.6 phonon — (1) **`a=l_P`**; (2) выписать факты H; (3) что может задать размер **без** инъекции **`α`**; (4) ratio — отчёт.
**Уже выведено (не свободные параметры):**
| # | факт | откуда |
|---|------|--------|
...
| H2 | Hop-лестница: **`N_re --α-- N_c --α-- N_a0`**, **`α=N_c/N_a0`**, **`α²=N_re/N_a0`** | §7.4 + шаг шкалы (Бор) |
...
$$
зазор $\sim 10^3$ ppm относительно CODATA. Это **не** неопределённость $M$ (оно фиксировано теоремой), а грубость приближения $\alpha_0=\kappa/M$: оно игнорирует поправки мягкой грани §8.2·U0.

##### 5 · Точное $\alpha$

Постоянная тонкой структуры **$\alpha$** (не $\alpha_0$) — фундаменталы мягкой грани при $M=97$ (§8.2·U0): согласование с CODATA на уровне $\sim 10^{-5}$ ppm. Физический смысл coupling phase↔vacuum — §8.2·α·meaning; $\alpha_0=\kappa/M$ — лишь следствие кулоновской посадки на $F_0$.
...
| точное T | предпочтительная мягкая грань | ~−0.000068 ppm | **$\alpha$** §8.2·U0 |
...
| фотон = holonomy $n=0$ | **выведена** схема |
...
Дискретный RJ — **не закрыт**. Это только: BZ режет continuum-catastrophe. Сама формула $\langle E\rangle=k_BT$ на моде — классический угол **ещё открыто** Bose-занятости (§5). Квант моды $E=\hbar\omega$; при $\hbar\omega\gtrsim k_B T$ равнораспределение ломается — но мост $T\to\langle n\rangle$ в MODEL пока **не выведено**.
**4 · Излучение как процесс** (уже §5 аннигиляция; **формулы канала** — §5.0.3; **углы и поляризации 2γ** — §5.0.3·подблок):
$$
...
$$
— это **T-статистика** (грубый прибор + хаос из детерминизма, §2.1). $T$ в §5.3.1 — kinetic zigzag **macro-газа материи**, не выведено как температура фотонной ванны.
...
| hinge | статус | зачем |
|-------|--------|-------|
| $\mu=0$ ($N_\gamma$ **не** аддитивный инвариант как $Q$; свободный $\gamma$ стабилен) | **выведена** схема: рождение/поглощение с материей (напр. аннигиляция → 2γ) · A3 на энергии | иначе не Bose, а $\mu\neq0$ |
| A6 + хаос §2.1 → равновесная мера на T | hinge, не формула | путь к ансамблю, не $\langle n\rangle$ |
| $T_{\gamma}$ vs $T$ gas (§5.3.1) | открыто контакт | одна $T$ в Planck — не выведено |
| $\omega(k)\approx c\|k\|$ (IR) · DOS | открыто (есть только $\omega_D$) | без DOS нет $u(\omega)\,d\omega$ |
| $g_{\mathrm{pol}}=2$ (поперечные) | выведено §5.0.3·□ | фактор 2 в $u(\omega)$ |
...
| $\mu=0$ ($N_\gamma$ ≠ инвариант; $Q$ сохраняется; свободный $\gamma$ стабилен) | **выведена** схема |
| аннигиляция → 2γ (два фронта) | **выведена** схема · sim |
| дискретный RJ $\langle E\rangle=k_BT$ | T-статистика, не закрыт |
| Bose $\langle n\rangle$ · $u(\omega)$ · Stefan/Wien | открыто (T + DOS) |
| $T_{\gamma}$, $\omega(k)$, A6→мера | hinges выше |
| $g_{\mathrm{pol}}=2$; углы/поляризации 2γ (аннигиляция) | выведено §5.0.3 |
...
$$
**Эталон сопряжения (уже выведено):** **`κ(a)`** при **`a=l_P`** → **`hT=κ·t_P`**, **`E₀=κE_P`**, **`λ₀=κℓ_P`**. **`c=κc₀`** — **проверка readout**, не определение **`κ`** (§7.2). Не «нашли ratio в таблице» — **сначала длины тела**, потом **`R_in/R_out`**.
**Бум **`V=(16/3)v_hV`**** — следствие **двух объёмов при том же `a`**, не отдельный fit. Сопряжение с **`g`** — **открыто** (как **`κ`** уже сопряжён).
#### §8.2·geo  · **`a` → coupling**
**Подход:** (1) зафиксировать **`a=l_P`**; (2) выписать **размерные** величины тела; (3) спросить, **какая** входит в фазу/силу; (4) ratio — только отчёт.
...
| **`α`** | мягкая грань | fundamentals | закрыто | — |
#### §8.2·α·EM·faces · Вес граней / dihedral → α?
**Подход:** лист 1 — площади/углы при `a=l_P` → какая доля = сопряжение?
**Ответы:**
1. Тело: $w_\square\approx 0.634$, $w_\triangle\approx 0.366$, dihedral $3/4$, $V/(Sa)\approx 0.398$ — **выведено**.
...
| force | $\kappa=1/\sqrt{2}$ | $M\in\mathbb{N}$ | **κ закрыто**; **M=97** закрыто; предпочтительная мягкая грань; лабораторное согласование; unit descent **закрыто** |
...
$$
Грубое лабораторное: $\alpha_0=\kappa/M$ (~−1040 ppm). Точное $\alpha$ — предпочтительная мягкая грань при $M=97$ (§8.2·U0).
...
Структурная α закрыта (мягкая грань). **Вверх — закон каскада (exact):**
```
v   = α^N_hier · E_P · √(2π)
...
Мягкие полы ~10⁻³ (packing / планковская дырка) — высшая структура, не пробел в α.
