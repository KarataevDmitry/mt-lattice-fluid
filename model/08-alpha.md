...
**Спектроскопия:** «тонкая структура» — мелкие **расщепления** линий (релятивистские и смежные поправки к уровням). Позже то же слово закрепили за **безразмерным** EM-coupling в форме Соммерфельда $\alpha=e^2/(4\pi\varepsilon_0\hbar c)$.

**Почему имя намекало на костыль.** Удобная линейка СИ уже несёт $e$, $\hbar$, $c$, $\varepsilon_0$; $\alpha$ получают **размерной комбинацией** «собрать безразмерное отношение». Это не логическая ошибка, но **перевёрнутая онтология**: coupling выглядит *производным* от T-якорей, хотя по смыслу это первичное **отношение квантов** (на M: $F_{\mathrm{Coulomb}}(N{=}1)=F_0/M$, §8.2·full-quant; смысл phase↔vacuum — §8.2·α·meaning).

**Канон (порядок слоёв):**
| шаг | объект | роль |
|-----|--------|------|
| 1 | $\alpha_{fs}$ | M-число из $g$ / геометрии носителя (§8.2·U0) |
| 2 | $e_0$ | T-якорь (измеренный элементарный заряд), не *вход* в определение $\alpha$ |
| 3 | $\alpha=e^2/(4\pi\varepsilon_0\hbar c)$ | **согласование** лабораторное описание T с тем же coupling |
| — | «fine structure» спектра | феномен уровней; **не** синоним определения $\alpha_{fs}$ |

**Связь с единицами:** после time-first триады $\{t_P,l_P,m_P\}$ и carrier $hT$ все SI-метры/секунды — T-export; $\alpha$ остаётся **точной безразмерной** и задаёт hop-лестницу EM-длин ($N_{a0}=N_c/\alpha$, §8.2·meter-fint), а не наоборот.

**Глоссарий:** `00-glossary.md` → **α (постоянная тонкой структуры)**.

#### §8.2·α·dim · Размерный анализ lab-формулы
Оси СИ: **L, M, T, I** (ток — для заряда).
...
| $\hbar$ | $\mathrm{ML^2T^{-1}}$ |
| $c$ | $\mathrm{LT^{-1}}$ |
| $\varepsilon_0$ (из $F=q_1 q_2/(4\pi\varepsilon_0 r^2)$) | $\mathrm{M^{-1}L^{-3}T^{4}I^{2}}$ |

Проверка:
\[
[\varepsilon_0\hbar c]=\mathrm{T^{2}I^{2}},\qquad
\left[\frac{e^2}{4\pi\varepsilon_0\hbar c}\right]
=\frac{\mathrm{I^{2}T^{2}}}{\mathrm{T^{2}I^{2}}}=1.
\]
Coupling **обязан** быть безразмерным; формула Соммерфельда — не «новая единица», а **сжатие** $e,\hbar,c,\varepsilon_0$ в одно число. На M то же число задаётся без $e$ (§8.2·α·name); размерный анализ относится к **T-export**, не к онтологии $M$.

**Книга:** `\cref{rem:alpha-fs-dim}` · **код приборов:** `α` в `QuantityKind` = dimensionless, SI через time-first ladder, не через подстановку $e$.
...
**π-guardrail (§8.2·geo):** непрерывный **`4π`** — **макроописание T** (телесный угол), **не** метрика одной **`v_h`**. На **1-tick** теле **кубооктаэдра** живут **6□ + 8△**, **`V=(16/3)v_hV`**, **`κ=1/√2`** — см. **`SI.cuboctahedron_geometry_row()`**.
...
$$
|F| = F_0/M,
\qquad |F|/F_P = \kappa/M = \alpha\ \text{(макроописание T)}.
$$
...
| sim plaquette на макроописание T | открыто |
...
| кусок | статус |
|-------|--------|
| макро-уравнения = макроописание планк. слоя | выведено |
...
Фотон на M = безмассовая мода бегущей фазовой голономии ($n=0$, $\Phi_\square$-фронт). Macro-макроописание — binomial $(1{-}2{-}1)$ по многим плакеткам (§4.1 · T1):
$$
...
**Не** отдельный Lagrangian. Излучение = **сектор $n=0$** той же жидкости + макроописание на T.
...
\quad
\text{Вероятность / }\langle n\rangle\ \text{— макроописание T, не микро-шаг.}
$$
...
**Следствие:** дискретный RJ / Planck **не закрыты**. Закрыты лишь: квант, бозонный сектор $n=0$, конечный набор мод. Формулы $\langle n\rangle$, $u(\omega)$, Stefan — требуют макроописание T.
...
$$
**Эталон сопряжения (уже выведено):** **`κ(a)`** при **`a=l_P`** → **`hT=κ·t_P`**, **`E₀=κE_P`**, **`λ₀=κℓ_P`**. **`c=κc₀`** — **проверка макроописание**, не определение **`κ`** (§7.2). Не «нашли ratio в таблице» — **сначала длины тела**, потом **`R_in/R_out`**.
