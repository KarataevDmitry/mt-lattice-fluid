...
### §6.0 · Полная лестница M → T (все шаги шкалы до макромира)

**Зачем.** Один взгляд: от планкона до метра — **каждый** шаг шкалы как длина **$\lambda$** в квантах **$\ell_P=a$** (шаг решётки $a$; в коде `hL`) и, где выведено, в степенях **`α_fs`**.
...
| **(dh, dt)** | native coords $dh=a$, $dt=\Delta t$; `c₀=dh/dt`, `E₀=s₀/dt` (код: hL, hT) | §3.12.6b |
| **шаги шкалы k** (ниже) | $\lambda_k$, якорь Compton **`λ₀=λ_C`**, **`k=0`** | эта таблица |
| **кванты `s₀→E₀`** | механика **на одном** `hV` | §5.2.2 |

**Буквы осей (`N` — только кольцо и именованные отношения; длина — $\lambda$; время — $t$, $\tau$):**

| символ | смысл | лог в тексте |
|--------|--------|--------------|
| **$\lambda$, $L$, $a$, $\ell_P$** | $a$ — шаг решётки; $\ell_P:=a$ | `log₁₀(λ/ℓ_P)` в таблице шкалы |
| **$\tau$, $\Delta t$** | номер такта; $t_{\mathrm{SI}}(\tau)=\tau\,\Delta t$ (§3.0.1; код `hT`) | `log₁₀` в космологии |
| **`N_ring`** | $|\mathbb{Z}_{512}|$, фазовое кольцо | **`log₂`** через $B_{hV}$ |

Код/verify: `N_today`, `log10_N_today` = $\tau_{\mathrm{today}}$; `N_c` = $\lambda_C/\ell_P$ (исторические имена полей).

**Две оси на одной лестнице длины:**
$$
\frac{\lambda}{\ell_P}=\frac{L}{\ell_P},\qquad
\lambda_C=\bar\lambda_C(e),\quad
N_c:=\frac{\lambda_C}{\ell_P}=\frac{m_P}{m_e}\approx 2.3\times 10^{22}.
$$
- **Геометрия звезды:** $\lambda=N_{12}^{m}\ell_P$ ($m=0,1,2,\ldots$) — составные оболочки **до** IR.
- **IR / EM (hop-лестница, §8.2·α·H2):** от якоря $\lambda_C$ вверх по длине
$$
\lambda_{k+1}=\frac{\lambda_k}{\alpha_{\mathrm{fs}}},\qquad
\lambda_{-1}=\alpha_{\mathrm{fs}}\,\lambda_C\;(r_e),\quad
\lambda_0=\lambda_C,\quad
\lambda_1=\frac{\lambda_C}{\alpha}\;(a_0),\quad \lambda_2=\frac{\lambda_C}{\alpha^2},\ \ldots
$$
- **Массы (степени α, §8.2):** $v\sim\alpha^{8}$, $m_H\sim\alpha^{8}$, $m_p\sim\alpha^{1}v$, $m_e\sim\alpha^{2}m_H$, $m_\nu\sim\alpha^{5}m_H$ — **энергия**, не длина; длина того же тела: $\lambda=\hbar/(Mc)=\ell_P\,m_P/M$.

**Порядок величин** ($\log_{10}(\lambda/\ell_P)$ при $N_{12}=12$, $\alpha^{-1}\approx 137$):

| k | $\lambda/\ell_P$ | $\log_{10}(\lambda/\ell_P)$ | степень α / масса | объект | слой | статус |
|---|-----------------|--------------|----------------|--------|------|--------|
| **0** | $1$ | $0$ | $m_P$ | планкон (`hV`), $n\in\mathbb{Z}$, $E_0\cdot\mathbb{Z}$ | M | **закрыто** |
| **1** | $N_{12}=12$ | $1.1$ | — | ε-звезда; $R_{\mathrm{dress}}=1\cdot dl$ | M | **закрыто** |
| **2** | $N_{12}^2=144$ | $2.2$ | — | 2-я коорд. сфера | M | геом. |
| **3** | $N_{12}^3=1728$ | $3.2$ | — | floor1 окно (C4 пре-резонанс) | M | §6·floor1* |
| **4** | $N_{12}^4=2.07\times 10^4$ | $4.3$ | — | floor1 полоса | M | **в работе** |
| **5** | $N_{12}^5\approx 2.5\times 10^5$ | $5.4$ | — | верх floor1; C2 единственная устойчивая $B{=}0$ материя | M | census **закрыто** |
| **6** | $\sim 10^{15}$ | $15$ | confining | floor2: кварковые foci ($\sim d{=}3$) | M | схема |
| **7** | $N_{-1}=\alpha N_c\sim 1.7\times 10^{20}$ | $20.2$ | $k{=}{-}1$ | $r_e$; hadron / packing | M·T | hop **закрыто** |
| **8** | $N_0=N_c\sim 2.3\times 10^{22}$ | $22.4$ | $k{=}0$ | $\bar\lambda_C(e)$; $m_e$ выведено | M→T | масса **закрыто** |
...
| **15** | $\lambda\gg\sigma_R\,dl$ | — | — | **макромир**: Newton, гладкая волна (Thm **T-CR**, §4.1.0-T) | T | readout **вынужден** |

Строки **k≥10** — **продолжение** $N_k=N_c/\alpha^k$ (порядок; не fitted knobs). Точные целые $N_k$ — когда hop-лестница и $N_{\mathrm{pack}}$ закрыты (§8.2·H).
...
**Код (leftovers):** `SI.floor1_leftovers_close_row()` · verify **`Floor1_leftovers_close`** — существование C3-канала закрыто; $\nu$ off-band.
...
**Закрыто на M:** форма часов = $n_{\mathrm{ticks}}\in\mathbb{N}\cdot hT$ (не float $\Gamma$).
