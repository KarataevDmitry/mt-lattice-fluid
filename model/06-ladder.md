...
**Зачем.** Один взгляд: от кирпича до метра — **каждый** шаг шкалы в hops `N·dl` и, где выведено, в степенях **`α_fs`**.
...
| **шаги шкалы k** (ниже) | длина в **`N·dl`**, якорь Compton **`k=0`** | эта таблица |
...
| k | $N$ (hops `dl`) | $\log_{10}N$ | степень α / масса | объект | слой | статус |
|---|-----------------|--------------|----------------|--------|------|--------|
