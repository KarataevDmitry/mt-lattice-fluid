...
**Зачем.** Один взгляд: от кирпича до метра — **каждый** рунг в hops `N·dl` и, где выведено, в степенях **`α_fs`**.
...
| **(dh, dt)** | native coords `dh=hL`, `dt=hT`; `c₀=dh/dt`, `E₀=s₀/dt` | §3.12.6b |
...
|---|-----------------|--------------|----------------|--------|------|--------|
| **0** | $1$ | $0$ | $m_P$ | кирпич `hV`, pra-дефект, $n\in\mathbb{Z}$, $E_0\cdot\mathbb{Z}$ | M | **закрыто** |
