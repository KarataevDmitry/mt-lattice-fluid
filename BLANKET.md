# ISM blanket · вторая поверхность (T readout)

**Океан (первая поверхность):** ST-ткань M-слоя (~$10^{31}$ K в log readout) — SSOT **`MODEL.md`** + hub [`model/`](model/) + код **`mt_ca.app`** (`HabitatPreset.VACUUM_BOIL`, `habitat_boil`).
...
| | **MODEL** | **BLANKET** | **META** |
|---|-----------|-------------|----------|
| Роль | SSOT **`g`**, M→T мост | SSOT **второй поверхности** (МЗВ на океане) | UI / космология **над** readout |
