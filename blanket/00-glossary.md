# Glossary · ISM blanket

**SSOT терминов** для `BLANKET.md` и dogfood. Код (`apply_ism_blanket`, `T_K_warm_nominal`) не переименовывается без миграции yaml.

| термин | смысл |
|--------|--------|
| **Океан (ST)** | Первая поверхность: `VACUUM_BOIL` / `boil_ocean_spinor_3d` — живая решётка до excitations. SSOT: MODEL + `mt_ca.app`. |
| **Одеяло (МЗВ)** | Вторая поверхность: несколько z-слоёв над интерфейсом с exp(−τ) от однородного τ; readout T в kK. |
