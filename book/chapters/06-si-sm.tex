...
\begin{proof}
\emph{Микроуровень: $\alpha^*$ --- не $\alpha_{\mathrm{fs}}$.}
...
\[
  ...
  \qquad
  \alpha^*=1+\frac{1}{4\pi}\approx 1{,}08.
\]
Число $\alpha^*$ --- числитель gate при $\rho_E\to 0$ на~$M$;
это \emph{не} константа тонкой структуры ($\alpha_{\mathrm{fs}}\approx 1/137$).
Коэффициент $1/(4\pi)$ --- доля полного фазового оборота на одном микрошаге,
а не $|N|^{-1}$.

\smallskip
\emph{Макроуровень: $\alpha_{\mathrm{fs}}$.}
Константа тонкой структуры --- безразмерная сила электромагнитного сопряжения
на осреднённом уровне~$T$: $e^2/(4\pi\varepsilon_0\hbar c)$.
При emergent-$3$D осреднении (контуры $\Phi_\square$, \cref{ch:macro})
полный телесный угол $4\pi$ стерадиан входит как множитель объёма фазового пространства,
а не как $|N|=12$:
\[
  \alpha_{\mathrm{fs}}^{-1}=4\pi^3+\pi^2+\pi\approx 137{,}036,
  \qquad
  \alpha_{\mathrm{fs}}\approx 0{,}00730.
\]
...
Связь двух уровней --- через общий телесный угол $4\pi$ и фазовый бюджет,
а не через тождество $\alpha^*=\alpha_{\mathrm{fs}}$.
\end{proof}
