...
\[
  ...
  N_{\mathrm{ring}}=512.
\]
Один тик кольца --- приращение $\tau$ на единицу по модулю~$N_{\mathrm{ring}}$;
Heisenberg-пол $\Delta\phi_{\mathrm{disc}}=41$ тиков соответствует
...
\[
  ...
  =\frac{41}{512};
\]
половина кольца (Паули) --- $N_{\mathrm{ring}}/2=256$ тиков.
...
\section{Внутренние координаты ячейки \texorpdfstring{$(k_\phi,\rho_{\mathrm{f}},\mu)$}{(kphi,rhof,mu)}}
...
Фазовый тик $\tau\in\mathbb{Z}_{N_{\mathrm{ring}}}$ раскладывается по Heisenberg-кванту
$\Delta\phi_{\mathrm{disc}}=41$:
\[
  \tau \equiv k_\phi\cdot\Delta\phi_{\mathrm{disc}}+\rho_{\mathrm{f}}
  \pmod{N_{\mathrm{ring}}},
\]
где $k_\phi\in\mathbb{Z}_{N_\phi}$ --- сектор ($0,\ldots,12$),
$\rho_{\mathrm{f}}\in\mathbb{Z}_{\Delta\phi_{\mathrm{disc}}}$ --- тонкая фаза ($0,\ldots,40$).
Амплитуда на решётке $Q(2^{-B_{\mathrm{amp}}})$, $B_{\mathrm{amp}}=6$:
$\mu\in\mathbb{Z}_{64}$ --- наименьший ненулевой квант $|z|$ на ячейке.
\end{definition}

\begin{proposition}[Каноническая пара $(k_\phi,\rho_{\mathrm{f}})$]
\label{prop:canonical-phase}
Для каждого $\tau\in\mathbb{Z}_{512}$ существует единственная каноническая пара
с минимальным $k_\phi$ при $0\le\rho_{\mathrm{f}}<41$, восстанавливающая $\tau$.
...
\[
  ...
  =13\cdot 41-21.
\]
Пары $(k_\phi=0,\,\rho_{\mathrm{f}}<21)$ и $(k_\phi=12,\,\rho_{\mathrm{f}}\ge 21)$
описывают один и тот же тик кольца.
\end{proposition}

\begin{proof}
Прямой перебор $512$ значений $\tau$ и проверка биекции канонической формы
...
$\rho_{\mathrm{f}}=0$, $\tau=k_\phi\cdot 41$ --- тринадцать ориентаций кирпича,
а не произвольная фаза на кольце.
Эволюция~$g$ заполняет весь $\mathbb{Z}_{512}$.
\end{remark}

Полный спинор: две компоненты $j=1,2$, каждая $(\mu_j,k_{\phi,j},\rho_{\mathrm{f},j})$
или $(\mu_j,\tau_j)$; в вакууме $k_{\phi,1}=k_{\phi,2}$, $\rho_{\mathrm{f},1}=\rho_{\mathrm{f},2}=0$.
Относительная фаза SU(2) --- $\tau_1-\tau_2$; динамика на шве из $21$ тика остаётся открытой
...
\midrule
$\tau$, $k_\phi$, $\rho_{\mathrm{f}}$, $\mu$; $dh$, $dt$; $s_0$ & $\hbar$, $t_P$, $\ell_P$, $c$ \\
$\Delta\phi_{\mathrm{disc}}/N_{\mathrm{ring}}$ & $\alpha_*-1=1/(4\pi)$ \\
$N_{\mathrm{ring}}/2$ тиков & угол $\pi$ \\
$\log_2 N_{\mathrm{ring}}+\varepsilon_B$ & $B_{\hv}=2\pi/\ln 2$ \\
$N_\phi=13$ & $N_\phi=\lceil 4\pi\rceil$ \\
$k=\Delta x/dh$, $\omega=2\pi/\htick$ & волновой вектор и частота в СИ \\
\bottomrule
\end{tabular}
\end{table}
...
(CR, \cref{ch:evolution}); шов из $21$ тика --- геометрия фазовой фибрации
